...
const ArticleCard = ({ article }) => {
  ...
  return (
    <div onClick={() => handleCardClick(article._id)} className="cursor-pointer">
      <img src={article.image || '/default-image.png'} alt={article.title} className="object-cover w-full h-40 rounded-lg" />
      <div className="mt-4">
...
const SidebarItem = ({ article }) => (
  <li className="flex items-center space-x-2 hover:bg-gray-100 p-2 rounded-md">
    <img src={article.image} alt={article.title} className="object-cover w-20 h-20 rounded-lg" />
    <a href="#" className="text-black">{article.title}</a>
  </li>
);

const CustomerArticle = ({ searchTerm }) => {
  const { data: articles, loading, error } = useFetchData(`${Base_URL}/blog`);
  const [filteredArticles, setFilteredArticles] = useState([]);
  const [tags, setTags] = useState([]);
  const [selectedTags, setSelectedTags] = useState([]);
  const [blogType, setBlogType] = useState('customer'); // You can change this value
  ...
  }, [searchTerm, articles, selectedTags]);
              ...
              <div className="flex flex-wrap gap-4">
                {tags.length > 0 ? (
                  tags.map(tag => (
                    <button 
                      key={tag} 
                      className={`px-4 py-2 text-sm border rounded-lg ${selectedTags.includes(tag) ? 'bg-gray-300' : 'bg-white border-black text-black'}`} 
...
export default CustomerArticle;
