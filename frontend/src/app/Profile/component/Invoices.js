  ...
  return (
    <div className="max-w-screen-lg p-4 mx-auto sm:p-8">
      ...
      <div className="p-4 bg-white shadow-md sm:p-8" ref={targetRef}>
        ...
        {new Date(Invoices.checkInDate).toLocaleDateString()} - {new Date(Invoices.checkOutDate).toLocaleDateString()}
        </div>
        <div>Number of persons</div>
        <div>{Invoices.numberOfPersons} adults</div>
        <hr className="h-2 my-2 "/>
          
        <div>Diet</div>
        <div>{Invoices.diet || "No special requests"}</div>
      </div>
